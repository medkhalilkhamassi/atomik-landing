  ...
  return (
    <>
      ...
      >
        ...
        {/* Folder title */}
        <h3
          className="text-lg font-semibold text-foreground mt-4 transition-all duration-300 text-center px-4"
          style={{
...
export const ProjectCard = forwardRef<HTMLDivElement, ProjectCardProps>(
  ({ image, title, delay, isVisible, index, onClick, isSelected, transparent }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "absolute w-20 h-28 rounded-lg overflow-hidden",
          !transparent && "shadow-xl bg-card border border-border",
          isSelected && "opacity-0",
        )}
        style={{
          transform: isVisible
            ? `translateY(-90px) translateX(${translations[index]}px) rotate(${rotations[index]}deg) scale(1) translateZ(0)`
            : "translateY(0px) translateX(0px) rotate(0deg) scale(0.5) translateZ(0)",
          ...
          outline: "1px solid transparent", // Hack for anti-aliasing
